class Course < ApplicationRecord
  ...
  def next_lesson(current_user)
    ...
    completed_lessons = current_user.lesson_users.includes(:lesson).where(completed:true)
    .where(lessons: {course_id: self.id})
    started_lessons = current_user.lesson_users.includes(:lesson).where(completed:false)
    .where(lessons: {course_id: self.id}).order(:position)
